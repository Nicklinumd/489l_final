...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default='result/best_model.pth.tar', type=str)
    ...
    model.eval()
    # for image in list("data/vae_test/normal"):
    #     image = image.cuda()
    #     print(image)
    #     print(get_vae_score(vae=model, image=image))
    # for image, target in enumerate(test_loader):
            # cls = classes[target.item()]
        # if args.cuda:
    resultPath = "data/vae_test/result"
    normalPath = "data/vae_test/result/normal"
    lesionPath = "data/vae_test/result/lesion"
    ...
    def outlier_detect(dir):
        ...
        for image in os.listdir(dir):
            ...
            x = TF.to_tensor(img)
            x.unsqueeze_(0)
            score = compute_all_scores(vae=model, image=x)
            print(score)
            if score['vae_score'] > 50:
                copyfile(imgPath, os.path.join(lesionPath, image))
            else:
                copyfile(imgPath, os.path.join(normalPath, image))

    outlier_detect("data/vae_test/lesion")
    outlier_detect("data/vae_test/normal")
